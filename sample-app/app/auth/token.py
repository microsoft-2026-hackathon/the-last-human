...
#: Maximum refresh calls, including the first attempt.
MAX_REFRESH_ATTEMPTS = 3

#: Base delay between refresh attempts, in seconds. Grows linearly.
...
async def refresh(transport: Transport, token: TokenSet) -> TokenSet:
    ...
    Failures are raised as-is. The transport layer retries transient failures
    within each refresh call.
    """
    ...
    for attempt in range(1, MAX_REFRESH_ATTEMPTS + 1):
        try:
            ...
            if attempt == MAX_REFRESH_ATTEMPTS:
                return token
            await asyncio.sleep(REFRESH_RETRY_BACKOFF_SEC * attempt)
