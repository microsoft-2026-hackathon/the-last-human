...
해커톤 발표용 덱입니다. **88초 데모 영상을 가운데 두고 앞 70초 · 뒤 52초**를 슬라이드가 맡습니다.

작년 우승작(MCP Shield)의 발표 문법을 적용했습니다 — 고객의 질문으로 열고, 요소를 하나씩 쌓아 올리고, 결론을 맨 마지막에 그리고, 실물 CTA로 닫습니다.
...
| `M` | 타이머 + 하단 진행바 숨기기 — 화면 공유 전에 끄세요 |
| `H` | 좌측 하단 안내 숨기기 |
| `Home` / `End` | 처음 / 본편 마지막 |

### 빌드 단계

슬라이드는 우승작처럼 **요소가 하나씩 쌓입니다.** 타이머에 `단계 2/6`으로 현재 위치가 표시됩니다.

| 슬라이드 | 단계 | 무엇이 쌓이나 |
| --- | --- | --- |
| 01 고객의 질문 | 3 | 질문 6개 → 가운데 결론 → 기술부채 문장 |
| 04 여섯째 줄 | 6 | 초록 체크가 한 줄씩 켜지고 → **여섯째 빈 줄이 점멸** |
| 05 방향 반전 | 3 | 4단계 → 언제 발동하나 → 무엇이 다른가 |
| 07 우리가 멈췄다 | 3 | 숫자 4개 **카운트업** → 가설·실험 → 측정·학습 |
| 08 아키텍처 | 6 | GitHub → Actions → Server → 작성자·모델 → **상태 화살표가 그려짐** → 스택 띠 |
| 10 한계 | 3 | 30/40 점수판 → 개선안 A·B·C → 원인 |

**리허설에서 손가락이 꼬이면 `↓`를 쓰세요.** 빌드를 건너뛰고 슬라이드가 완성된 상태로 넘어갑니다.
