...
**AI가 코드를 만들어도, 변경을 이해하고 책임질 사람은 필요하다.** The Last Human은 중요한 PR에서 작성자의 설명과 코드 근거를 연결하고, 그 확인을 머지 조건과 프로젝트의 모듈별 커버리지로 이어 준다.
...
| --- | --- | --- | --- | --- |
| 00:00-00:06 | AI가 작성해도 책임은 사람에게 남는다. | [연출] 일반적인 AI 코드 생성 장면이 진행되다가 머지 직전에 멈춘다. 기계적인 리듬을 사용하되 제품 기능·로고·처리량 수치를 나열하지 않는다. | AI can write the code. People still carry the responsibility. | 합성 개발 화면. 실제 업무 처리량이나 성능 측정처럼 표현하지 않는다. |
| 00:06-00:14 | 유지보수 현장에서 실제 기여 규칙이 바뀌고 있다. | [근거] [Godot의 정책 발표][godot-policy] 제목과 기여자의 이해·수정 책임을 강조한 부분만 확대한다. 여러 프로젝트가 모든 AI 코드를 금지한다는 표현은 하지 않는다. | Godot is tightening contribution rules: people must understand their code and be able to fix it. | 공식 발표의 제목·짧은 발췌·출처 표시. 기여 제한의 범위와 발표 문맥을 유지한다. |
...
| 00:22-00:26 | 누가 이 변경을 이해하고 책임질 것인가. | [연출] 자료 화면과 기계적인 리듬이 사라진다. 머지 직전 모니터만 남기고 질문 뒤 짧은 정적을 둔다. | Who will understand this change and take responsibility? | 기능 설명 없이 한 문장에 집중한다. |
...
- [Godot 발표][godot-policy]는 리뷰 부담과 기여자의 이해·수정 책임, AI 기여 제한을 직접 다룬다. 우리 게이트가 해당 프로젝트의 기여 정책을 대체한다는 메시지는 아니다.
