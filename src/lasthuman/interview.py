...
질문은 두 축을 **모두** 덮어야 합니다.
  ...
  structure 축  이 변경이 무엇 위에 서 있고, 어디까지 미치는가
    structure   : **변경된 코드가 호출하는 다른 파일의 심볼**이 실제로 무엇을 하는지 —
                  몇 번 하는지, 어떤 조건에서 하는지, 그것이 이 변경과 겹치면 무슨 일이
                  나는지 — 를 묻는다. 그 심볼이 이미 반복·재시도·캐시·잠금·타임아웃을
                  하고 있다면 이 변경과의 **합성 효과**(예: 한 요청이 상대에 최대 몇 번
                  도달하나)가 질문이다. 구조 사실에 "호출하는 다른 파일의 심볼"이 없을
                  때에만 호출자·임포터·실패가 번지는 범위를 묻는다.
                  아래 "구조 사실"에 실제로 있는 내용만 근거로 삼는다

{n}개 중 최소 1개는 structure 여야 합니다. 구조 사실이 "없음"뿐이면 code로만 채웁니다.
구조 사실에 "호출하는 다른 파일의 심볼"이 있으면 **"누가 이 함수를 부르나 / 어느 파일이
영향을 받나" 류의 질문은 만들지 않습니다** — 그건 PR 설명만 읽어도 답할 수 있어 게이트가
무력해집니다.

모든 질문은 **객관식 4지선다**입니다.

보기 규칙 — 이걸 어기면 게이트가 무력해집니다
- 정답은 정확히 하나. 네 보기는 **서로 배타적인 주장**이어야 한다 — 정답과 같은 사실을 다른 말로
  되풀이한 보기(예: "3회 × 3회"와 "최대 9회")가 있으면 정답이 둘이 되어 채점이 무너진다.
  숫자·횟수를 묻는 질문이면 각 보기의 수치가 서로 달라야 한다
...
- {n}개 중 **최소 1개는 답이 hunk 밖에** 있어야 한다 — "구조 사실"에 적힌 다른 파일의 동작이나
  상수를 알아야 답할 수 있는 질문. 구조 사실이 "없음"이면 전부 hunk 안으로 채운다
- hunk 밖 질문의 재료는 **우선 "변경된 코드가 호출하는 다른 파일의 심볼"과 그 파일의 상수**다 —
  호출된 것이 실제로 무엇을 하는지, 몇 번 하는지, 이 변경과 겹치면 무슨 일이 나는지.
  그 사실이 있으면 반드시 그것을 묻고, evidencePath는 그 파일이다. 호출자·임포터 목록은
  피호출자 사실이 없을 때의 차선이다
- hunk 안 질문은 **consequence를 우선**한다 — 실패 경로나 경계 조건에서 무엇이 반환되거나
  발생하는지. 코드 위치를 짚게만 하는 claim 은 그다음이다
